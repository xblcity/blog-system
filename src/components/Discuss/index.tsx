...
import AuthModal from '@components/BlogLayout/components/AuthModal'
import { publishComment } from '@services/api'
import {
  ArticleItem,
  CommentItem,
  ReplyItem
} from '@views/ArticleList/ArticleItem'
import DiscussList from './DiscussList'
import { AuthModalType } from '@components/BlogLayout/components/UserInfo'
...
const Discuess = ({ articleData }: IProps) => {
  ...
  const renderMenu = () => {
    return !!userInfo ? (
      ...
      <Menu>
        <MenuItem onClick={() => triggerAuthModal(true, 'login')}>
          登录
        </MenuItem>
        <MenuItem onClick={() => triggerAuthModal(true, 'register')}>
          注册
        </MenuItem>
      </Menu>
    )
