...
const UserManager = () => {
  ...
  const { list, total, page, loading } = useGetListData<
    UserItem,
    FetchParams.GetUserList
  >(getUserList, params, cancelRequire)
  ...
  const changePageAndSorter = (
    ...
    sorterStatus: SorterResult<UserItem>
  ) => {
        ...
        {({ height }) => {
          return (
            <Table
              ...
              dataSource={list}
              onChange={(p, f, s: SorterResult<UserItem>) =>
                changePageAndSorter(p, s)
              }
            >
              <Column<UserItem>
                width={200}
